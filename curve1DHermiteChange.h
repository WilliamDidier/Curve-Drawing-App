#ifndef CURVE_1D_HERMITE_CHANGE_H
#define CURVE_1D_HERMITE_CHANGE_H
...
class Curve1DHermiteChange : public Curve1D {
 public:
 Curve1DHermiteChange(const QString &name) : Curve1D(name) {}
 Curve1DHermiteChange(Curve1D *curve,const QString &name) : Curve1D(curve,name) {}
  ...
  float derivate(Vector2f p1, Vector2f p2){
    float res = sin((p2[1]-p1[1])/(p2[0]-p1[0]));
    return res;
  }
...
class Curve1DHermiteChangeConstructor : public Curve1DConstructor {
 public:
  virtual ~Curve1DHermiteChangeConstructor()                    {}
  virtual const QString type() const                      { return "HermiteCurveChange";             }
  virtual Curve1D *create(const QString &name)            { return new Curve1DHermiteChange(name);   }
  virtual Curve1D *create(Curve1D *c,const QString &name) { return new Curve1DHermiteChange(c,name); }
};
