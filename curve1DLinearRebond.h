#ifndef CURVE_1D_LINEARREBOND_H
#define CURVE_1D_LINEARREBOND_H
...
class Curve1DLinearRebond : public Curve1D {
 public:
 Curve1DLinearRebond(const QString &name) : Curve1D(name) {}
 Curve1DLinearRebond(Curve1D *curve,const QString &name) : Curve1D(curve,name) {}
  ...
  QPainterPath path(float xmin,float xmax) {
    ...
    for (unsigned int i=1;i<nbPts();++i) {
      ...
      for (uint j = 0; j < N; j++){
        p.lineTo(pt_tmp[0]+(pt_tmp2[0]-pt_tmp[0])*(1-1/pow(2,j)),pt_tmp2[1]-(1-pow(-1,j+1))*((pt_tmp2[1]-pt_tmp[1])/pow(2,j+1)));
      }
...
class Curve1DLinearRebondConstructor : public Curve1DConstructor {
 public:
  virtual ~Curve1DLinearRebondConstructor()              {}
  virtual const QString type() const                      { return "RebondLinearCurve";             }
  virtual Curve1D *create(const QString &name)            { return new Curve1DLinearRebond(name);   }
  virtual Curve1D *create(Curve1D *c,const QString &name) { return new Curve1DLinearRebond(c,name); }
};


#endif // CURVE_1D_LINEARREBOND_H
